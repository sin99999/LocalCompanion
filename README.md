...
1. **言語選択**（日本語 / English / Español / Português / Русский / 简体中文 / 한국어）
2. **初回セットアップ**（**先に** GGUF フォルダの有無。無いときだけ既定モデルを取得）
3. **自動準備**（プログレス表示）

### お手持ちの GGUF がある場合（推奨・先に聞きます）

初回セットアップ画面で **GGUF フォルダを選ぶ** と、指定フォルダ内の GGUF を読み取り専用で利用できます（フォルダ内のファイルは変更しません）。複数ある場合は **ファイルサイズが最小のチャット用 GGUF** を自動選択します。既定モデルのダウンロードは行いません。
...
お使いの GPU を確認し、適した設定で llama-server を起動します。この経路の初回は **インターネット接続** が必要です。

2 回目以降は、すでに準備済みの場合はすぐ使い始められます。
...
| 資料検索（RAG） | 登録したテキスト資料を検索し、回答に反映します。スキャン画像のみの PDF は OCR 未対応です |
| Web 参照 | メッセージ内の URL 自動取得、「調べて」「ネットから探して」などの調査意図での Web 検索（キー不要。オフ可） |
| チャット書き出し | 「デスクトップに保存して」などと依頼すると、回答をテキストファイルとして保存します（依頼検出後に保存します。同名ファイルがあるときはチャットで上書き／別名を確認します。キャラクター設定の提案とは別で、設定ファイルの書き換え確認ダイアログは出ません） |
| 会話からキャラクターを育てる | **設定 → キャラクター** で有効化できます（既定オフ）。提案は確認ダイアログで承認したときだけキャラクター設定ファイルへ反映します |
| 長期記憶 | 名前付きキャラクターごとに短い事実を PC 内に保存し、**今の話題と関連するときだけ**自然に触れることがあります（あいさつなど無関係な短文では差し込みません。一覧表示・手動追加はありません。**設定 → 基本設定**）。デフォルト AI には長期記憶はありません |
| 音声入力 | Windows の音声認識で入力（**設定 → 基本設定** で有効化） |
| モデル管理 | 付属 `models\` と、任意で指定する **追加モデルフォルダ** を一覧表示。適用で llama を再起動 |
| 読み上げ | VOICEVOX で返答を音声再生（任意） |
| 画面言語 | 日本語 / English / Español / Português / Русский / 简体中文 / 한국어（足りない文言は日本語。アプリ内ヘルプ HTML は日本語と英語） |
...
URL 添付・メッセージ内 URL の取得上限は **2 MB** です。Web 検索で開く各ページは最大約 **512 KB** です（いずれも RAG 取込とは別の上限です）。メッセージに `https://...` を書くと自動で内容を取り込みます。「調べて」「ネットから探して」などの調査意図では、キー不要の Web 検索（既定: DuckDuckGo HTML）も行います（`appsettings.json` の `WebSearchEnabled` でオフにできます）。ただし **刑法・労基法など法令条文や「資料から検索」** の話は、Web ではなく登録資料（RAG）を優先します。
...
チャット書き出しの保存先は、依頼に応じて Desktop / Documents / Downloads / USB / 指定フォルダなどです。「アプリのフォルダに」など exe 横を指定した場合は、`data\exports\`（または LocalAppData 側の `exports\`）へ保存します。Windows や Program Files 配下などへの書き込みは拒否します。
...
**Q. 「すでに起動しています」と出ます。**  
A. LocalCompanion は同時に 1 つだけ起動できます。先に開いているウィンドウをご利用ください。何も見えない場合は、タスクバー・別のデスクトップ・最小化をご確認ください。見つからない場合は、タスクマネージャーで LocalCompanion を終了してから再起動してください。
...
A. 初回セットアップ、または **設定 → モデル選択** の「追加モデルフォルダ」で指定できます。指定フォルダは読み取り専用で、mmproj はアプリ側の `models\` に取得します。
...
| 症状 | まず確認すること |
|------|------------------|
| 起動しない | [.NET 10 Desktop Runtime（x64）](https://dotnet.microsoft.com/download/dotnet/10.0) をインストールします。「すでに起動しています」と出る場合は、タスクバー・別のデスクトップ・最小化を確認してください。見つからない場合はタスクマネージャーで LocalCompanion を終了してから再起動してください |
| チャット画面が空／真っ白 | [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/) が入っているか（必須） |
| チャットが使えない | **設定 → モデル選択** の状態が「OK」になるまで数分待つ。ポート 8080 が他アプリや残った llama-server に使われていないか |
