...
URL 添付・メッセージ内 URL・調査時の Web ページ取得の上限は **2 MB** です（RAG 取込とは別の上限です）。メッセージに `https://...` を書くと自動で内容を取り込みます。「調べて」などの調査意図では、キー不要の Web 検索（既定: DuckDuckGo HTML）も行います（`appsettings.json` の `WebSearchEnabled` でオフにできます）。
