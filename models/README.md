...
- **手動** … 下記の手順で任意の GGUF を `models/` に配置、または **設定 → モデル選択** の「追加モデルフォルダ」で外部フォルダを指定
...
> **VRAM** … 6〜8 GB 級なら **7B Q4_K_M** から。メモリ不足のときは量子化を下げ、**設定 → モデル選択** でコンテキスト長を下げて **`LocalCompanion.exe` を再起動**してください。
...
- **モデル** で mmproj を自動選択（初回はアプリが補完を試行）

- チャットの **画像を挿入** が使えれば vision 利用可（mmproj が正しく設定されている場合）

- **設定 → モデル選択** の実行状態が **OK** であることも確認してください（実行状態に「画像入力 OK」という表示はありません）
...
| 一覧に出ない | 拡張子 `.gguf`、設定を開き直す、**モデルを適用（llama 再起動）** |
